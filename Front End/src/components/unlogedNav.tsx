...
<Box display={{xs : 'flex' , sm : 'none'}} >    {
      mobileNavButtonState ? <Stack direction="column" sx={{width : '100vw' , position : 'absolute' , backgroundColor: '#000'  , alignItems : 'center' ,zIndex : 2 , height : '90vh' , top : '0px' , left : '0px' }} spacing="32px" justifyContent="center"  >
...
<Link  style={{textDecoration : 'none'}} to="/regester" onClick={()=>{dispatch({type: 'Reverse'})}}  > <StyledTypography   variant='h4' sx={(theme)=>({borderBottom : location.pathname === '/regester' ? '1.5px solid white'  : 'none'  , color : location.pathname === '/about' ?'secondary.light' : '#fff'})} >Regester</StyledTypography></Link>
<Link  style={{textDecoration : 'none'}}  to="/about" onClick={()=>{dispatch({type: 'Reverse'})}}  ><StyledTypography  variant='h4' sx={(theme)=>({borderBottom : location.pathname === '/about' ? '1.5px solid white'  : 'none' , color : location.pathname === '/regester' ?'secondary.light' :'#fff' })} >About</StyledTypography></Link>
